...
class SupCsTrainer(Trainer):
    ...
    def compute_loss(
        ...
        if self.pooling_strategy == 'pooler':
            # For token classification, use mean of logits
            logits = output.logits.mean(dim=1, keepdim=True)
        else:
            logits = output.logits.mean(dim=1, keepdim=True)
        ...
        for p_dpr in self.w_drop_out:
            ...
            # ---- concat logits ------#
            if self.pooling_strategy == 'pooler':
                logits = torch.cat((logits, model(**inputs).logits.mean(dim=1, keepdim=True)), 1)
            else:
                logits = torch.cat((logits, model(**inputs).logits.mean(dim=1, keepdim=True)), 1)
        ...
        if p_dpr != self.def_drop_out: 
            model = self.set_dropout_mf(model, w=self.def_drop_out)
        
        # SupContrast
        loss_fn = SupConLoss(temperature=self.temperature_s) 
